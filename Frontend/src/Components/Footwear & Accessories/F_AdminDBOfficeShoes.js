...
import axios from '../../utilities/axios'; // Make sure the path to axios is correct
import { useNavigate } from 'react-router-dom';

const F_AdminDBOfficeShoes = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await axios.post('/api/officeshoes', formDataWithImage, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        console.log("Response:", response); // Debugging: Log server response
        alert("Office shoes item has been added successfully.");
        setFormData({
          sellerNo: "",
          itemNo: "",
          name: "",
        ...
        alert(`There was an error adding the office shoes item: ${error.response?.data.error || error.message}`);
      }
    }
  };
  ...
  return (
    <div style={styles.formBackground}>
      <div style={styles.container}>
        <h2 style={styles.heading}>Add New Office Shoes Item</h2>
...
export default F_AdminDBOfficeShoes;
