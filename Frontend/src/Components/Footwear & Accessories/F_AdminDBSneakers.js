...
const F_AdminDBSneakers = () => {
    ...
    switch (name) {
      ...
      case "quantity":
        newErrors[name] = !value || isNaN(value) || value <= 0
          ? `${name.replace(/([A-Z])/g, ' $1').trim()} must be a positive number and not empty.`
          : "";
        break;
      case "name":
  ...
  const validateForm = () => {
    ...
    if (!formData.sellerNo || isNaN(formData.sellerNo)) {
      newErrors.sellerNo = "Seller No must be a positive number and not empty.";
    }
    if (!formData.itemNo || isNaN(formData.itemNo)) {
      newErrors.itemNo = "Item No must be a positive number and not empty.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      formDataWithImage.append("quantity", formData.quantity); // Add quantity here
      formDataWithImage.append("image", imageFile);

      try {
        const response = await axios.post('/api/sneakers', formDataWithImage, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        console.log("Response:", response); 
        alert("Sneakers item has been added successfully.");
        setFormData({
          sellerNo: "",
          itemNo: "",
          name: "",
        ...
        setImageFile(null);
        setErrors({});
        navigate('/AdminDBSB'); // Redirect to a success page or list page
      } catch (error) {
        console.error("Error submitting the form:", error.response?.data || error.message);
        alert(`There was an error adding the sneakers item: ${error.response?.data.error || error.message}`);
      }
    }
  };
  ...
  return (
    <div style={styles.formBackground}>
      <div style={styles.container}>
        <h2 style={styles.heading}>Add New Sneakers Item</h2>
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          ...
          <button type="submit" style={styles.button}>Add Sneakers Item</button>
        </form>
      </div>
    </div>
  );
...
export default F_AdminDBSneakers;
