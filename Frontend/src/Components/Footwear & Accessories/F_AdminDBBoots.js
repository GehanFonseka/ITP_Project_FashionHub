...
const F_AdminDBBoots = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await axios.post('/api/boots', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        console.log("Response:", response); // Debugging: Log server response
        alert("Boots item has been added successfully.");
        setFormData({
          sellerNo: "",
          itemNo: "",
          name: "",
        ...
        setImageFile(null);
        setErrors({});
        navigate('/AdminDBBoots'); // Redirect to a success page or list page
      } catch (error) {
        console.error("Error submitting the form:", error.response?.data || error.message);
        alert(`There was an error adding the boots item: ${error.response?.data.error || error.message}`);
      }
    }
  };
  ...
  return (
    <div style={styles.formBackground}>
      <div style={styles.container}>
        <h2 style={styles.heading}>Add New Boots Item</h2>
