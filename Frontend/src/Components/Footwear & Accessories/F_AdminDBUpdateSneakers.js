...
const F_AdminDBUpdateSneakers = ({ sneakers, onClose, onUpdate }) => {
  ...
  useEffect(() => {
    if (sneakers) {
      setFormData({
        sellerNo: sneakers.sellerNo || '',
        itemNo: sneakers.itemNo || '',
        name: sneakers.name || '',
        price: sneakers.price || '',
        description: sneakers.description || '',
        image: sneakers.image || '',
        quantity: sneakers.quantity || '', // Set initial quantity from sneakers data
      });
    }
  }, [sneakers]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/sneakers/${sneakers._id}`, formData);
      onUpdate(); // Notify parent component of the update
      onClose(); // Close the modal
    } catch (error) {
      console.error("Error updating sneakers item:", error);
      setError("Failed to update sneakers item. Please try again.");
    }
  };
...
export default F_AdminDBUpdateSneakers;
