...
import C_AdminDBUpdateTMMensBlazer from './C_AdminDBUpdateTMMensBlazer'; 

const C_AdminDBTMMensBlazerTable = () => {
  ...
  const fetchBlazerData = async () => {
    try {
      const response = await axios.get('/api/tm-mensblazers');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/tm-mensblazers/${id}`);
  ...
  return (
    <div>
      <h2>Tailor Made Men's Blazer Inventory</h2>
...
export default C_AdminDBTMMensBlazerTable;
