...
const C_AdminDBUpdatePants = ({ pants, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/pants/${pants._id}`, formData);
      onUpdate(); // Notify parent component of the update
      onClose(); // Close the modal
    } catch (error) {
      console.error("Error updating pants item:", error);
      setError("Failed to update pants item. Please try again.");
    }
  };
...
export default C_AdminDBUpdatePants;
